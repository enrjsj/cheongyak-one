...
public interface MemberSavedSearchProfileRepository extends JpaRepository<MemberSavedSearchProfile, Long> {
    ...
    @Query("select profile from MemberSavedSearchProfile profile join fetch profile.member member where member.active = true and profile.newNoticeEnabled = true")

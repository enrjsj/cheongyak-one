...
import com.cheongyakone.infrastructure.external.reb.RebApartmentUnitTypeClient;
...
/** APT 본공고가 저장된 뒤 주택형별 공급·분양가를 보강한다. 실패해도 본공고 동기화를 실패시키지 않는다. */
@Service
public class RebApartmentUnitTypeSyncService {
    private static final Logger log = LoggerFactory.getLogger(RebApartmentUnitTypeSyncService.class);
    private final RebApartmentUnitTypeClient client;
    ...
    public RebApartmentUnitTypeSyncService(RebApartmentUnitTypeClient client, SubscriptionNoticeRepository noticeRepository, SubscriptionNoticeUnitTypeRepository unitTypeRepository) {
        this.client = client;
